...
namespace dso
{
                                                  ...
                                                  ImmaturePointTemporaryResidual* residuals)
  {
    ...
    {
      if(print) printf("OptPoint: OUTLIER numGoodRed %d, minObs %d !\n", numGoodRes, minObs);
      return (PointHessian*)((long)(-1));		// yeah I'm like 99% sure this is OK on 32bit systems.
                                                        // comments: why use so much c style in c++????????
    }
    ...
    if(!std::isfinite(p->energyTH)) {
      if(print) printf("OptPoint: Infinite energyTH!\n");
      delete p; return (PointHessian*)((long)(-1));
    }
    ...
    for(int i=0;i<nres;i++)
      ...
      {
        printf("residuals[i].state_state is %d\n", residuals[i].state_state);

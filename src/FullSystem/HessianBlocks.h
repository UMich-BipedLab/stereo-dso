...
#define SCALE_B_INVERSE (1.0f / SCALE_B)
  ...
  // hold the image array contents. The pose info are in struct FrameShell
  struct FrameHessian
  {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    EFFrame* efFrame;
  ...
  // the points that have meaningful depth info, and can be tracked in other frames
  struct PointHessian
  {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    static int instanceCounter;
    ...
    int idx;
    float energyTH;
    FrameHessian* host;  // host keyframe
    bool hasDepthPrior;
